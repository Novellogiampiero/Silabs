...
static int autecradio_init_clocks(void);

static int autecradio_init(void)
{
	int ret;

#ifdef CONFIG_SOC_GECKO_DEV_INIT
	autecradio_init_clocks();
#endif
...
SYS_INIT(autecradio_init, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE);
