...
////////LOG_MODULE_REGISTER(fg2x, CONFIG_BOARD_EFR32G24_LOG_LEVEL);

static int fg23_init_clocks(void);

static int fg23_init(void)
{
	int ret;
	...
	fg23_init_clocks();
#endif
	static struct gpio_dt_spec wake_up_gpio_dev =GPIO_DT_SPEC_GET(DT_NODELABEL(wake_up_trigger), gpios);


	if (!gpio_is_ready_dt(&wake_up_gpio_dev)) {
		printk("Wake-up GPIO device was not found!\n");
		return -ENODEV;
	}
...
SYS_INIT(fg23_init, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE);
